...
var SchemaMigrations = map[string][]string {
	...
	"0.0.2":[]string{
		...
		"create table if not exists map (name text primary key, source text not null, category text not null)",
		"create table if not exists match (id integer primary key, category text not null, player1 text not null, player2 text not null, commit1 text not null, commit2 text not null, map text not null, result text not null, created timestamp not null default current_timestamp, updated timestamp default null, replay text default null, unique (category, map, player1, player2, commit1, commit2))",
	},
}
